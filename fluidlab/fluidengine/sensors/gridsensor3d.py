...
class GridSensor3D(GridSensor):
    ...
    def compute_lat_lon_target(self, s: ti.i32):
        for i in range(self.n_particles):
            ...
            self.targets[s, i].latitudes = 180 - lat_rad * (
                    180.0 / ti.acos(-1.0))  # acos(-1) is a way to get π in Taichi
    ...
    def normal_distance_particle(self, s: ti.i32):
        ...
        for p in range(self.n_particles):
            if self.particle_used[s, p] != 0:
                if self.particles[s, p].distance < self.m_MaxDistance and self.particles[
                    s, p].distance > self.m_MinDistance:
                    # 2. 判断经度范围和纬度范围
                    if (90 - self.particles[s, p].latitudes < self.m_LatAngleSouth and 90 - self.particles[
                        s, p].latitudes >= 0) or \
                            (ti.abs(self.particles[s, p].latitudes - 90) < self.m_LatAngleNorth and ti.abs(
                                self.particles[s, p].latitudes - 90) >= 0):
                        if ti.abs(self.particles[s, p].longitudes) < self.m_LonAngle:
                            # 计算加权距离
                            d = (self.particles[s, p].distance - self.m_MinDistance) / (
                            ...
                            ti.atomic_max(self.grid_sensor[s, latitude_index, longitude_index, self.sim.particles_i[
                                p].body_id].distance, normal_d)
    @ti.kernel
        ...
        for p in range(self.n_particles):
            if self.particle_used[s, p] != 0:
                if self.targets[s, p].distance < self.m_MaxDistance and self.targets[
                    s, p].distance > self.m_MinDistance:
                    # 2. 判断经度范围和纬度范围
                    if (90 - self.targets[s, p].latitudes < self.m_LatAngleSouth and 90 - self.targets[
                        s, p].latitudes >= 0) or \
                            (ti.abs(self.targets[s, p].latitudes - 90) < self.m_LatAngleNorth and ti.abs(
                        ...
                        if ti.abs(self.targets[s, p].longitudes) < self.m_LonAngle:
                            ...
                            latitude_index = ti.cast(
                                ti.floor(
                                    (self.targets[s, p].latitudes - (90 - self.m_LatAngleSouth)) / self.m_CellArc),
                                ti.i32)

                            # 使用 atomic_max 更新 normal_distance 的值
                            ti.atomic_max(self.grid_sensor[s, latitude_index, longitude_index, -1].distance, normal_d)
    @ti.kernel
    def get_sensor_data_kernel(self, s: ti.i32, grid_sensor: ti.types.ndarray()):
        # 这里假设 output 已经是一个正确维度和类型的 Taichi field
        for i, j, k in ti.ndrange(self.M, self.N,
                                  self.n_bodies):
    ...
    def get_obs(self):
        ...
        # grid_sensor = grid_sensor[..., 0]
        # grid_sensor = grid_sensor.permute(1, 0)
        # grid_sensor = torch.flip(grid_sensor, [0])
        # grid_sensor = grid_sensor.unsqueeze(0).unsqueeze(-1)
        # cv2.imshow('3d grid sensor', grid_sensor.detach().cpu().numpy()[..., 1])
        # cv2.waitKey(1)
        return grid_sensor
