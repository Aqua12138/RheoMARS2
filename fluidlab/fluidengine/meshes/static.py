...
class Static(Mesh):
    ...
    def __init__(self, **kwargs):
        self.position = ti.Vector.field(3, DTYPE_TI, shape=(1), needs_grad=True)  # positon of the effector
        self.quatation = ti.Vector.field(4, DTYPE_TI, shape=(1), needs_grad=True)  # quaternion for storing rotation
        self.quatation.from_numpy(np.array([[1, 0, 0, 0]]))
    ...
    def set_pos(self, position, quatation):
        self.position.from_numpy(position)
        self.quatation.from_numpy(quatation)
    ...
    def update_vertices(self):
        for i in self.vertices:
            self.vertices[i] = ti.cast(
                geom_utils.transform_by_trans_quat_ti(self.init_vertices[i], self.position[0], self.quatation[0]),
                self.vertices.dtype)
            self.vertex_normals[i] = ti.cast(
                geom_utils.transform_by_quat_ti(self.init_vertex_normals[i], self.quatation[0]), self.vertices.dtype)

    @ti.func
    def sdf(self, pos_world):
        # sdf value from world coordinate
        pos_mesh = geom_utils.inv_transform_by_trans_quat_ti(pos_world, self.position[0], self.quatation[0])
    ...
    def normal(self, pos_world):
        ...
        normal_vec_world = geom_utils.transform_by_quat_ti(normal_vec_mesh, self.quatation[0])
