...
@ti.data_oriented
class SDFDensitySHACLoss(Loss):
    def __init__(
            self,
            type,
            **kwargs,
        ):
        super(SDFDensitySHACLoss, self).__init__(**kwargs)
        self._gamma = 0.99
        self.gamma = ti.field(dtype=DTYPE_TI, shape=(), needs_grad=False)
        self.rew = ti.field(dtype=DTYPE_TI, shape=(self.max_loss_steps,), needs_grad=True)
        self.rew_acc = ti.field(dtype=DTYPE_TI, shape=(self.max_loss_steps,), needs_grad=True)
    ...
    def build(self, sim):
        ...
        self.inf = 1000
        super(SDFDensitySHACLoss, self).build(sim)
    ...
    def iou_kernel(self) -> ti.float64:
        ...
        return I / (U - I)
    def reset_grad(self):
        super(SDFDensitySHACLoss, self).reset_grad()
